...
export async function runStrategist(
  ...
  // 5. Build the prompt
  const systemPrompt = `You are the Strategist Agent for a multi-brand media newsroom. Your job is to decide which brand(s) should create content for a given narrative, on which platform(s), and with what specific angle.
...
5. Prioritize quality over quantity — only create decisions where there's a genuine editorial fit.
...
If no brand is a good fit for this narrative, return an empty decisions array with a brief explanation.`;
