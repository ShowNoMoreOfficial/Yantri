...
export async function POST() {
  try {
    ...
    const { stdout, stderr } = await execAsync(
      `python "${scriptPath}"`,
      {
        env: { ...process.env, GEMINI_API_KEY: geminiKey, PYTHONUTF8: "1" },
        timeout: 300000, // 5 minute timeout (RSS + Gemini ranking takes ~3-4 min)
        maxBuffer: 10 * 1024 * 1024, // 10MB buffer for Unicode-heavy output
        encoding: "utf8",
      }
    );
